...
const generateInvoicePDF = (orderInfo) => {
  return new Promise((resolve, reject) => {
    ...
    doc.font('Roboto-Bold').fontSize(26).fillColor('#FFFFFF')
      .text('INVOICE', MARGIN, 30);

    doc.font('Roboto').fontSize(10).fillColor('rgba(255,255,255,0.85)')
    ...
    doc.font('Roboto-Bold').fontSize(8).fillColor(GRAY)
      .text('BILLED TO', MARGIN + 16, y + 12, { characterSpacing: 1 });
    doc.font('Roboto-Bold').fontSize(11).fillColor(DARK)
    ...
    doc.font('Roboto-Bold').fontSize(13).fillColor(RED)
    ...
    if (orderInfo.shippingAddress) {
      ...
      const titleH = addr.title
        ? doc.font('Roboto-Bold').fontSize(10).heightOfString(addr.title, { width: textW })
        : 0;
      const linesH = addrLines.reduce((sum, line) =>
        sum + doc.font('Roboto').fontSize(9).heightOfString(line, { width: textW }) + 4, 0);
      ...
      addrLines.forEach(line => {
        const lh = doc.font('Roboto').fontSize(9).heightOfString(line, { width: textW });
