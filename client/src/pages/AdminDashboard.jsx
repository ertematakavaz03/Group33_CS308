                ...
                <tbody>
                  {products.map(p => (
                    <tr key={p.id} style={{ borderBottom:"1px solid #f3f4f6" }}>
                      ...
                      <td style={{ padding:"1rem", fontWeight:"600", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{p.name}</td>
                      <td style={{ padding:"1rem", color:"#6b7280" }}>{p.category}</td>
                      <td style={{ padding:"1rem", fontWeight:"700", color:"#b91c1c" }}>${parseFloat(p.price).toFixed(2)}</td>
                      <td style={{ padding:"1rem" }}>
                        <span style={{ background: p.stock<=0?"#fee2e2":p.stock<=5?"#fef3c7":"#dcfce7", color:p.stock<=0?"#dc2626":p.stock<=5?"#d97706":"#16a34a", padding:"4px 12px", borderRadius:"20px", fontSize:"0.8rem", fontWeight:"700" }}>{p.stock}</span>
                      </td>
                      <td style={{ padding:"1rem" }}>
                        <button onClick={() => openEdit(p)} style={{ background:"#f3f4f6", border:"none", padding:"0.5rem 1rem", borderRadius:"6px", cursor:"pointer", fontWeight:"700", marginRight:"0.5rem" }}>Edit</button>
                        <button onClick={() => handleDelete(p.id)} style={{ background:"#fee2e2", color:"#dc2626", border:"none", padding:"0.5rem 1rem", borderRadius:"6px", cursor:"pointer", fontWeight:"700" }}>Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody>
                  {users.map(u => (
                    <tr key={u.id} style={{ borderBottom:"1px solid #f3f4f6" }}>
                      <td style={{ padding:"1.2rem 1rem", color:"#6b7280" }}>#{u.id}</td>
                      <td style={{ padding:"1.2rem 1rem", fontWeight:"700", color: "#111827" }}>{u.name}</td>
                      <td style={{ padding:"1.2rem 1rem", color:"#4b5563" }}>{u.email}</td>
                      <td style={{ padding:"1.2rem 1rem", color:"#6b7280" }}>{u.created_at ? new Date(u.created_at).toLocaleDateString() : "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
const thStyle = { textAlign:"left", padding:"1rem", color:"#6b7280", fontWeight:"700", fontSize:"0.8rem", textTransform:"uppercase", borderBottom:"2px solid #f3f4f6" };
